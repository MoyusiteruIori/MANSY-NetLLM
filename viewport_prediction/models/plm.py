...
from transformers import BertModel, GPT2Model, PreTrainedModel
import torch
...
class ViewportPretrainedLM(nn.Module):
    def __init__(
        ...
        self.d_model = 768      # both gpt2 and bert-base have hidden size 768
        ...
        self.plm = peft_model(
            GPT2Model.from_pretrained("openai-community/gpt2").to(device=self.device) if plm_type == "gpt2" else \
                BertModel.from_pretrained("google-bert/bert-base-uncased").to(device=self.device),
            plm_type=plm_type,
            rank=rank,
            verbose=True,
        )
